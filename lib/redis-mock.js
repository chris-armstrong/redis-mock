 ...
 */
var DefaultInstance = new RedisMock();
var MockedInstanceHolders = {};
module.exports = exports = DefaultInstance;

/**
 * RedisClient constructor
 */
function RedisClient(stream, options, instance) {
  ...
  this._instance.on('message', this._message);
...
var end = RedisClient.prototype.end = function () {
  ...
  this._instance.removeListener('message', this._message);
...
RedisClient.prototype.publish = function (channel, msg) {
  pubsub.publish.call(this, this._instance, channel, msg);
}
...
RedisClient.prototype.get = RedisClient.prototype.GET = function (key, callback) {
  ...
  stringfunctions.get.call(this, this._instance, key, callback);
};

RedisClient.prototype.getset = RedisClient.prototype.GETSET = function (key, value, callback) {

  stringfunctions.getset.call(this, this._instance, key, value, callback);
};
...
RedisClient.prototype.set = RedisClient.prototype.SET = function (key, value, callback) {
    ...
    var expireTime = 0;
    var keyExists = false;
    if (key in this._instance.storage) {
        keyExists = true;
    }
    ...
    if(isEx === true) {
        if(isXx === true) {
            if(keyExists === true) {
                stringfunctions.set.call(self, self._instance, key, value, function () {
                  keyfunctions.expire.call(self, self._instance, key, expireTime, function(err, result) {
                      callback(err, "OK");
                  });
                });
            } else {
                self._instance._callCallback(callback, null, 0);
            }
        } else if(isNx === true) {

            if(keyExists === true) {
                self._instance._callCallback(callback, null, null);
            } else {
                stringfunctions.set.call(self, self._instance, key, value, function () {
                  keyfunctions.expire.call(self, self._instance, key, expireTime, function(err, result) {
                      callback(err, "OK");
        ...
        if(isXx === true) {
            if(keyExists === true) {
                stringfunctions.set.call(self, self._instance, key, value, callback);
            } else {
                self._instance._callCallback(callback, null, null);
            }
        } else if(isNx === true) {
...
RedisClient.prototype.ping = RedisClient.prototype.PING = function (callback) {
  ...
  stringfunctions.ping.call(this, this._instance, callback);
};

RedisClient.prototype.setex = RedisClient.prototype.SETEX = function (key, seconds, value, callback) {
	var self = this;
  stringfunctions.set.call(self, self._instance, key, value, function () {
    keyfunctions.expire.call(self, self._instance, key, seconds, function(err, result) {
      self._instance._callCallback(callback, null, "OK");
    });
  });
};

RedisClient.prototype.setnx = RedisClient.prototype.SETNX = function (key, value, callback) {
  stringfunctions.setnx.call(this, this._instance, key, value, callback);
};

RedisClient.prototype.mget = RedisClient.prototype.MGET = function () {
  var newArguments = [this._instance];
  for (var i = 0; i < arguments.length; i++) {
...
RedisClient.prototype.hgetall = RedisClient.prototype.HGETALL = function (hash, callback) {
  ...
  hashing.hgetall.call(this, this._instance, hash, callback);
}
...
var bpop = function (fn, key, timeout, callback) {
  ...
  if (hasCallback) {
    fn.call(this, this._instance, keys, arguments[arguments.length - 2], arguments[arguments.length - 1]);
  } else {
    fn.call(this, this._instance, keys, arguments[arguments.length - 1]);
  }
}
...
RedisClient.prototype.lindex = RedisClient.prototype.LINDEX = function (key, index, callback) {
  listfunctions.lindex.call(this, this._instance, key, index, callback);
}

RedisClient.prototype.lrange = RedisClient.prototype.LRANGE = function (key, index1, index2, callback) {
  listfunctions.lrange.call(this, this._instance, key, index1, index2, callback);
}

RedisClient.prototype.lset = RedisClient.prototype.LSET = function (key, index, value, callback) {
  listfunctions.lset.call(this, this._instance, key, index, value, callback);
}
...
RedisClient.prototype.flushdb = RedisClient.prototype.FLUSHDB = function (callback) {
  ...
  serverfunctions.flushdb.call(this, this._instance, callback);
}
RedisClient.prototype.flushall = RedisClient.prototype.FLUSHALL = function (callback) {

  serverfunctions.flushall.call(this, this._instance, callback);
}

RedisClient.prototype.auth = RedisClient.prototype.AUTH = function (password, callback) {

  serverfunctions.auth.call(this, this._instance, password, callback);
}

RedisMock.prototype.createClient = function (port_arg, host_arg, options) {
	port_arg = port_arg || 6379;
	host_arg = host_arg || 'localhost';
	var name = host_arg.toLowerCase() + ':' + port_arg;
	if (name === 'localhost:6379') {
		return new RedisClient(null, null, DefaultInstance);
	}
	var mockedInstance = MockedInstanceHolders[host_arg.toLowerCase() + ':' + port_arg];
	if (!mockedInstance) {
		mockedInstance = new RedisMock();
		MockedInstanceHolders[name] = mockedInstance;
	}

  return new RedisClient(null, null, mockedInstance);
}
